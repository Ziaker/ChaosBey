...
export class CombatHarness {
  ...
  static async create(
    ...
    return new CombatHarness(physics, first, second, new RoundState());
  }

  tick(firstActions: ControllerActions, secondActions: ControllerActions): MatchTickResult {
    return tickMatch(this.physics, this.first, this.second, firstActions, secondActions, FIXED_DELTA_SECONDS, this.roundState);
  }
}
